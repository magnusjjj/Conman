<?php
	class TicketController extends Controller {
		...
		private function buildalternativetree()
		{
			...
			{
				if($alternative['ammount_compare'] >= $alternative['ammount'] && $alternative['ammount'] != 0)
				{
						if($alternative['template_override'] == 'ticket')
						{
							$this->set('ticket_disabled', true);
						} else {
						}
				} else {
		...
		private function checkmembership($member)
		{
		...
		function index()
		{
			...
			if(count($myorder))
			{
				...
				$this->set('ordersvalues', $ordersvalues->getOrderValuesFromOrder($myorder[0]['id']));
				$this->set('alternatives_simple', $this->tree_simple);
			} else {
				$this->buildalternativetree();
				$member = Model::getModel('member');
				$themember = $member->getMemberByUserID(Auth::user());

				$this->set('is_member', $this->checkmembership($themember));
				$this->set('alternatives_parents', $this->tree_parents);
				$this->set('alternatives_children', $this->tree_children);
			}
		}
		...
		function pay_return()
		{
			...
			$this->set('status', $paydetails->getStatus());
			$this->view = 'ticket.index.php';
			$this->index();
		}
		...
		function pay_status() // This does nothing, because of payson uncertainty
		{
			require_once(Settings::getRoot() . '/includes/paysonapi/lib/paysonapi.php');
			$order = Model::getModel('order');
