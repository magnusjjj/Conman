<?php
	class UserModel extends Model {
		...
		{
			return $this->db->query("SELECT * FROM users WHERE member_id = '%s'", $id);
		}
		...
		function editPass($id, $pass)
		{
			...
			$this->db->query("UPDATE users SET password='%s', salt='%s' WHERE member_id = '%s';", hash('SHA512', $pass . $thestring), $thestring, $id);
		}
